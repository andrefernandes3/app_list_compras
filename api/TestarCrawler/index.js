    ...
    try {
        ...
        for (const prod of monitorados) {
           ...
           // 2. TENTATIVA BUSCA (Fallback com Validação de Nome)
            if (!encontrado) {
                try {
                    const termo = encodeURIComponent(limparTermoBusca(prod.nome_comum).split(' ').slice(0, 2).join(' '));
                    ...
                    if (data && data.length > 0) {
                        ...
                        if (melhorProduto) {
                            ...
                            relatorio.resultados.push({ seu_item: prod.nome_comum, status: "NÃO ENCONTRADO (Nome não coincidiu)" });
                        }
                    } else {
